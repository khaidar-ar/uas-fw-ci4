...
class Produk extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach($this->produkModel->findAll() as $produk){
            $data['produk'][$i++] = [
                'produk' => $produk,
                'data_stok' => $this->stokModel->where('id_produk', $produk['id'])->findAll()
            ];
        }
    ...
    public function edit($urlSlug = false)
    {
        ...
        $data['produk']    = $this->produkModel->where('url_slug', $urlSlug)->first();
        if(!$data['produk']){
            return view('errors/errors-404');
        }
        $data['kategori']       = $this->kategoriModel->findAll();
        $data['stok']           = $this->stokModel->where('id_produk', $data['produk']['id'])->findAll();
    ...
    public function editSave()
    {
        ...
        return redirect()->to('dashboard/produk/edit/'.$produk['url_slug']);
    }
    ...
    {
        $data['produk']    = $this->stokModel->where('id', $id)->where('id_produk', $idProduk)->first();
        if(!$data['produk']){
            return view('errors/errors-404');
        }
        if($this->stokModel->where('id_produk', $idProduk)->countAllResults() < 2){
            ...
            if($this->stokModel->delete(['id' => $id])){
                session()->setFlashdata('success', 'Sukses menghapus variasi '.$data['produk']['ukuran']);
            }else{
                session()->setFlashdata('danger', 'Gagal menghapus variasi '.$data['produk']['ukuran']);
            }
        }
    ...
    {
        $data['produk'] = $this->produkModel->where('id', $id)->first();
        if(!$data['produk']){
            return view('errors/errors-404');
        }
        ...
        if($this->produkModel->where('id', $id)->delete()){
            unlink('assets/img/produk/'.$data['produk']['gambar']);
            session()->setFlashdata('success', 'Sukses menghapus produk '.$data['produk']['nama']);
        }else{
            session()->setFlashdata('danger', 'Gagal menghapus produk '.$data['produk']['nama']);
        }
