...
class Dashboard extends BaseController
{
    ...
    {
        $this->transaksiModel->select('*, transaksi.alamat_jalan AS alamat_pengiriman, transaksi.created_at AS tgl_pesan, transaksi.updated_at AS tgl_pembayaran');
        $this->transaksiModel->selectMin('id');
        $this->transaksiModel->groupBy('kode_trx');
        $this->transaksiModel->select('pelanggan.*');
        $this->transaksiModel->join('pelanggan', 'id_buyer = pelanggan.users_id');
    ...
    public function getAllTransactionByMonth($bln = "")
    {
        ...
        $this->transaksiModel->like('transaksi.updated_at', date('Y').'-'.$bln);
        $this->transaksiModel->orderBy('tgl_pembayaran', 'DESC');
        return $this->transaksiModel->get()->getResultArray();
    }
